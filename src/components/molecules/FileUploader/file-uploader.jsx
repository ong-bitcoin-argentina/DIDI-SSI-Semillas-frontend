...
const { uploadFile, uploadPdfFile ,validateSancorFile, uploadSancorFile } = apiCalls();
...
// pdfValidation={true}
const FileUploader = ({ buttonText, source, onChangeSource, onUploaded, onSuccessRequest,pdfValidation }) => {
  ...
  const handleUpload = async (createCredentials = false) => {
    ...
    try {
      ...
      let response;
      if(pdfValidation){
        uploadAction = source.name === 'sancor' ? uploadSancorFile : uploadPdfFile;
        response = await uploadAction(formData);

        // if (onSuccessRequest && response.data.downloadableFileName) {
        onSuccessRequest(response);
        // }
      } else {
        uploadAction = source.name === 'sancor' ? uploadSancorFile : uploadFile;
        response = await uploadAction(formData);
        if (onSuccessRequest && response.data.downloadableFileName) {
          onSuccessRequest(response);
        } else {
